---
title: Integrating depencency injection into a legacy code base
date: 2020-12-13 14:34:11
tags:
  - refactoring
  - software design
  - legacy code
...
But how do we deal with these singletons and the lack of IOC? Is there also a way to gradually introduce it, to create an island of sanity that can slowly grow over time?

In this article I'll show you how I dealt with this just recently. I used [Autofac](https://autofac.org/) because this has been my preferred container framework for years. But any other IOC framework that has a concept analogous to [Autofac's modules](https://autofaccn.readthedocs.io/en/latest/configuration/modules.html) should enable the same pattern.

## Initial situation
Let's start with what I found before the refactoring:

There was a type called `Repository`, split over 8 partial classes, all together coming in at around 7000 lines of code. That type was really not a repository at all, but a typical [God object](https://wiki.c2.com/?GodClass). It contained only static members. At least 30-40% of calls into that class were made by static members of other types or by singletons.

For reasons unrelated to this discussion, I had to change some of the functionality related to sending notifications (yes, I know - I told you it was a god-object) which was what prompted me to first make that portion of behavior testable.
...
## Create new type
So far so good, but it would still be an enormous drag to test changes to the `Send*` methods. We have to somehow separate the send functionality from `Repository` into a new type. 

To do this, we create a new type `Sender` and copy all related methods from `Repository` there. 

It may be tempting to move the methods right away, but trust me, it's not a good idea. In the many years I have been doing such refactorings, in at least 5 out of 10 times moving right away led me to have to go back and revert eventually, and re-start the process with copying, thus losing more time than if I'd copied from the start.
...
Unless you get very lucky, this is to be expected. Private members that are *only used by the `Send*` methods* are no problem, you can just copy them, too. But members that are used by some of the `Send*` methods *and* other members of `Repository` (or even exposed in `IRepository`) are more complicated. For the sake of this article, I will assume just one such member. If there are more, up to a certain limit, you can still go the same route. Beyond 4-5, though, you probably will have to group the methods you want to extract into another type according to the dependencies they have on `Repository`.
...
Once that is done, we still have a few issues to solve:
...
The solution for both issues is the really the same:
...
This can be done automatically with R#, updating all references correctly. 
...
With this we have solved the first of the problems mentioned further up, production code everywhere now uses our clean, well-tested `Sender`, 
and `Repository` doesn't know anything about sending anymore.


## Introduce IOC container
Finally, we can go on to introduce a container, with that solving the remaining two issues.
...
Over time, more and more types can be added as they are broken out from the god-object `Repository` (or elsewhere), any new type can 
be developed under the assumption of a container right away, and at some point in the future, when all types have been refactored to use IOC, 
we will be able to eliminate `Globals` itself, too.
...
Because `Lazy<>`'s value generation is intrinsically thread-safe and thus we can be sure that race-conditions do not lead to creation of multiple instances. This really is the kind of problem you wouldn't ever have if you used a container from the beginning, but as we are talking about a migration strategy from legacy code here, we must ensure thread-safety of singletons. 
...
The purist answer that this is a sign of bad design does not help in this context because this is really something that happens quite often for a while 
until refactoring of the legacy types has progressed far enough - in practice this can often mean many months because we don't get to refactor everything at once. (And even if we did, it wouldn't be wise to do it without intermediate steps, but that's a subject maybe for another article.)
...
This solves the circular dependency by changing the time when one of the two instances has to be created: instead of requiring an `ISender` instance at construction time of `Repository`, it is now only needed when `Repository` accesses it for the first time.

The decision whether to use `Lazy<>` or `Func<>` depends usually on whether the instance can change or not. If the instance is going to be created only once and effectively be a singleton for the duration of runtime, then `Lazy<>` is the better choice. If the instance can change, then `Lazy<>` would introduce bugs and `Func<>` is more correct.

On a side note, that both `Func<T>` and `Lazy<T>` can be resolved without any additional work, provided that `T` is registered, is one of the reasons why I like Autofac so much as a container. 
